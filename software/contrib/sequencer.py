...
class Sequencer(EuroPiScript):
    def __init__(self):
        ...
        self.image_blank = FrameBuffer(
            bytearray(b"\x00\x00\x00\x00\x00\x00\x00\x00\x00"), 3, 9, MONO_HLSB
        )
        self.step_image_selected = FrameBuffer(
            bytearray(b"\xa0@\xa0@\xa0@\xa0@\xa0"), 3, 9, MONO_HLSB
        )
    ...
    def b2_handler_falling(self):
        ...
        if (
        ...
        ):  # If displaying sequences, then the user has just chosen the sequence to edit
            ...
            self.editing_step
        ):  # If editing step, then the user has just chosen the new note value for the step
            self.sequence.set_step(self.selected_step, NOTES[self.currently_selected_step])
            self.editing_step = False
        elif (
            self.editing_sequence
        ):  # If editing sequence, then the user has just chosen the step to change the note value for
        ...
        for index, sequence in enumerate(self.sequences):
            ...
            if self.editing_sequence and sequence == self.sequence:
                ...
                for step in range(sequence.steps):
                    ...
                    if (
                        self.editing_step == False and step == self.currently_selected_step - 1
                    ) and (sequence == self.sequence):
                        image = self.step_image_selected
                    else:
                        if (
                            self.editing_step
                            and step == self.selected_step
                            and sequence == self.sequence
                        ):
                            step_image_index = ceil(
                                k2.read_position(len(NOTES), KNOB_SAMPLES) / 6.7
                            )
