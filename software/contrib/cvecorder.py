...
class CVecorder(EuroPiScript):
    ...
    def handleClock(self):
        ...
        if self.step < self.stepLength - 1:
            ...
            if self.CvRecording[self.ActiveCvr] == "true":
                ...
                self.saveState()
                if self.debugLogging:
                    self.writeToDebugLog(
                        f"[handleClock] Calling saveState() for bank {self.bankToSave}."
                    )
    ...
    def clearCvrs(self, bank):
        for b in range(self.numCVRBanks + 1):
            ...
            if self.debugLogging:
                self.writeToDebugLog(
                    f"[clearCvrs] Calling saveState() for bank {self.bankToSave}."
                )
    ...
    def saveState(self):
        ...
        for i in range(len(self.CVR[self.bankToSave])):
            self.CVR[self.bankToSave][i] = [
                int(x * 100) for x in self.CVR[self.bankToSave][i]
            ]
        ...
        while attempts < maxRetries:
            try:
                ...
                if self.debugLogging:
                    self.writeToDebugLog(
                        f"[saveState] Saving state for bank: {str(self.bankToSave)}. Size: {len(jsonState)}"
                    )
                ...
                with open(outputFile, "w") as file:
                    ...
                    if file.write(jsonState) > 0:
                        ...
                        if self.debugLogging:
                            self.writeToDebugLog(
                                f"[saveState] Bank {str(self.bankToSave)} saved OK"
                            )
                        break
                ...
                if self.initTest:
                    print(
                        f"[{attempts}] Error: Memory allocation failed, retrying: {e}"
                    )
                if self.debugLogging:
                    self.writeToDebugLog(
                        f"[saveState] Error: Memory allocation failed, retrying: {e}"
                    )
                    print(micropython.mem_info("level"))
                else:
                    pass
        ...
        for channel in self.CVR[self.bankToSave]:
            self.CVR[self.bankToSave][i] = [
                x / 100 for x in self.CVR[self.bankToSave][i]
            ]
            i += 1
            ...
            while attempts < maxRetries:
                ...
                try:
                    ...
                    with open(fileName, "r") as file:
                        ...
                        if self.initTest:
                            print(
                                f"Loading previous state for bank: {str(b)}. Size: {len(jsonData)}"
                            )

                        if self.debugLogging:
                            self.writeToDebugLog(
                                f"[loadState] [{attempts}] Loading previous state for bank: {str(b)}. Size: {len(jsonData)}"
                            )
                        ...
                        for channel in self.CVR[b]:
                            self.CVR[b][i] = [
                                x / 100 if x > 0 else 0 for x in self.CVR[b][i]
                            ]
                            i += 1
                    ...
                    if self.debugLogging:
                        self.writeToDebugLog(
                            f"[loadState] [{attempts}] No state file found for bank {b}. Error: {e}"
                        )
                    ...
                    if self.debugLogging:
                        self.writeToDebugLog(
                            f"[loadState] [{attempts}] Exception when attempting to open previous state file for bank {b}. {e}"
                        )
    ...
    def rotateLog(self):
        ...
        while self.logFileNum > 0:
            ...
            if self.currentLogFile in self.logFileList:
                os.rename(
                    self.currentLogFile,
                    self.logFilePrefix + str(self.logFileNum + 1) + ".log",
                )
            self.logFileNum -= 1
    ...
    def updateScreen(self):
        ...
        oled.rect(
            lPadding + 0, 0, int(self.CVR[self.ActiveBank][0][self.step] * 4), 11, 1
        )
        oled.rect(
            lPadding + 42, 0, int(self.CVR[self.ActiveBank][1][self.step] * 4), 11, 1
        )
        oled.rect(
            lPadding + 84, 0, int(self.CVR[self.ActiveBank][2][self.step] * 4), 11, 1
        )
        oled.rect(
            lPadding + 0, 12, int(self.CVR[self.ActiveBank][3][self.step] * 4), 11, 1
        )
        oled.rect(
            lPadding + 42, 12, int(self.CVR[self.ActiveBank][4][self.step] * 4), 11, 1
        )
        oled.rect(
            lPadding + 84, 12, int(self.CVR[self.ActiveBank][5][self.step] * 4), 11, 1
        )
